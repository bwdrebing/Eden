...
6. **Serialize it.** Add the state to the `useUrlSync("reflection", ...)`
   ...
   so renames are safe but should still be rare.) If a *default* you change
   would repaint somebody's saved scene, give the old value in the `legacy`
   argument instead, so links written before the setting existed reopen as
   their author left them.

7. **Renaming a workspace is a migration.** Every panel is gated on the tab
   id and `uiTab` rides in saved links, so a renamed id would open those
   scenes on an empty column. Add the old id to `LEGACY_TABS` (next to
   `WORKSPACES`); the normalizer resolves it and the URL heals itself the
   first time the link is opened.
